...
/**
 * 问题：
 * 假设现在有一个变量（股票的价格），一个线程需要不断的更新股票的价格，而另一个线程则需要根据满足的价格进行售卖。
 * 这便会带来一个问题就是，线程在更新股票的价格时，这个变量就会被一直lock住，这就会导致其他的线程访问不了这个变量。
 * 解决方法:
 * 引入条件变量： condition_variable
 */
#include <thread>
#include <iostream>
...
void PeterUpdateStock_Notify (StockSafe & safe, condition_variable& condition)
{
	...
	{
		{
			//创建了一个独占锁，保护了 safe 对象
			unique_lock<StockSafe> lock(safe);
			//从 safe 中获取了一个 StockBlackboard 类型的对象的引用,即resource
